...
const follow_count = 10;
	...
	it('should call the user search api and return the search results', (done) => {
		backend.connections.subscribe((connection: MockConnection) => {
			...
			expect(connection.request.url).toEqual(`http://api.loklak.org/api/user.json?`+
						`screen_name=${user}&followers=${follow_count}&following=${follow_count}&`+
						`callback=JSONP_CALLBACK&minified=true`);
	});
