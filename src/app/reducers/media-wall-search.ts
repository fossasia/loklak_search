 ...
 *
 * Here the reducer controls that part of the state which is shows the state of the application
 * wheather it is searching and what is it searching for.
 */
export function reducer(state: State = initialState, action: apiAction.Actions | wallAction.Actions | wallPaginationAction.Actions): State {
	switch (action.type) {
		case wallAction.ActionTypes.WALL_SEARCH: {
